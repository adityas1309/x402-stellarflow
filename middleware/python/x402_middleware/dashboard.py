"""
Embedded SEP-10 operator dashboard for x402_middleware (FastAPI version).

Mounts three routes on the user's FastAPI app:
    GET  /x402/dashboard   — single-page HTML/JS dashboard (Freighter login + stats)
    GET  /x402/stats       — JSON stats, gated by SEP-10 JWT
    (SEP-10 /api/auth/* routes must be mounted separately via sep10_auth.mount())

Usage:

    from fastapi import FastAPI, Request
    from x402_middleware import x402_paywall, sep10_auth, dashboard

    app = FastAPI()
    sep10_auth.mount(app)
    dashboard.mount(app, allowed_wallets=[os.getenv("X402_TREASURY_ADDRESS")])
...
def mount(
    app: Any,
    *,
    allowed_wallets: list[str] | None = None,
    dashboard_path: str = "/x402/dashboard",
    stats_path: str = "/x402/stats",
) -> None:
    """Register the dashboard HTML page + SEP-10-protected stats endpoint."""
    try:
        from fastapi import Request
        from fastapi.responses import HTMLResponse, JSONResponse
    except ImportError:
        raise ImportError("dashboard.mount() requires fastapi: pip install fastapi")

    from .sep10 import protect

    # Stats endpoint — protected by SEP-10 JWT
    @app.get(stats_path)
    @protect(allowed_wallets=allowed_wallets)
    async def x402_stats(request: Request):
        return default_logger.stats()
    ...
    async def x402_dashboard():
        return HTMLResponse(_dashboard_html(stats_path))


def _dashboard_html(stats_path: str) -> str:
    # Keep this HTML in sync with middleware/node/src/dashboard.ts
    return (
        """<!DOCTYPE html>
<html lang="en">
<head>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  button { background: var(--accent); color: var(--bg); border: none; padding: 10px 20px; border-radius: 8px; font-weight: 600; font-size: 14px; cursor: pointer; }
  button:hover { opacity: 0.9; }
  button.secondary { background: transparent; color: var(--text); border: 1px solid var(--border); }
  .center { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 60vh; text-align: center; gap: 14px; }
...
<main id="root">
  <div class="center">
    ...
    <div class="small">Powered by x402-middleware</div>
  </div>
</main>

<script type="module">
  const STATS_PATH = "__STATS_PATH__";
  ...
  if (jwt && walletAddr) { loadDashboard(); }

  window.signInFreighter = async () => {
    setError("");
    try {
      const freighter = await import("https://esm.sh/@stellar/freighter-api@6").catch(() => null);
      if (!freighter) throw new Error("Freighter not detected. Install https://freighter.app");
      ...
      const accountStr = typeof addr === "string" ? addr : addr.address;
      const chRes = await fetch(API_CHALLENGE + "?account=" + encodeURIComponent(accountStr));
      ...
      if (!signedXDR || typeof signedXDR !== "string") throw new Error("Freighter did not return a signed XDR");
  ...
  async function loadDashboard() {
    try {
      const res = await fetch(STATS_PATH, { headers: { Authorization: "Bearer " + jwt } });
  ...
  function renderDashboard(stats) {
    ...
    const recentRows = stats.recent.map((c) => {
      ...
      return `<tr><td class="mono">${t}</td><td>${c.endpoint}</td><td class="mono">${payerShort}</td><td class="amount">$${c.amount.toFixed(2)}</td><td class="mono">${c.facilitator}</td><td class="mono">${c.durationMs}ms</td></tr>`;
    }).join("");

    const byEpRows = stats.byEndpoint.map((e) =>
      `<tr><td>${e.endpoint}</td><td>${e.calls}</td><td class="amount">$${e.revenue.toFixed(2)}</td></tr>`
    ).join("");

    document.getElementById("root").innerHTML = `
      <div class="row">
        <div class="card"><div class="label">Total revenue</div><div class="value good">$${stats.totalRevenue.toFixed(2)}</div></div>
        <div class="card"><div class="label">Total calls</div><div class="value accent">${stats.totalCalls}</div></div>
        <div class="card"><div class="label">Unique payers</div><div class="value accent">${stats.uniquePayers}</div></div>
      </div>
      <div class="section-title">Revenue by endpoint</div>
      <div class="card" style="padding:0"><table><thead><tr><th>Endpoint</th><th>Calls</th><th>Revenue</th></tr></thead><tbody>${byEpRows || '<tr><td colspan="3" style="color:var(--dim);padding:16px">No paid calls yet.</td></tr>'}</tbody></table></div>
      <div class="section-title">Recent calls (last 50)</div>
      <div class="card" style="padding:0"><table><thead><tr><th>Time</th><th>Endpoint</th><th>Payer</th><th>Amount</th><th>Facilitator</th><th>Duration</th></tr></thead><tbody>${recentRows || '<tr><td colspan="6" style="color:var(--dim);padding:16px">No paid calls yet.</td></tr>'}</tbody></table></div>
    `;
  }

  window.signOut = () => { sessionStorage.clear(); location.reload(); };
  setInterval(() => { if (jwt) loadDashboard(); }, 5000);
</script>
</body>
</html>"""
    ).replace("__STATS_PATH__", stats_path)
